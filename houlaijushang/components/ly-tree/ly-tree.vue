	...
	<view>
		...
		<template v-else>
			<view v-if="isEmpty || !visible" class="ly-empty">
				<image src="@/static/img/nodata.png" mode="widthFix" class="nodata"></image>
				<view class="">
					{{emptyText}}
				</view>
				
			</view>
			<view :key="updateKey" class="ly-tree" :class="{'is-empty': isEmpty || !visible}" role="tree" name="LyTreeExpand">
				<ly-tree-node v-for="nodeId in childNodesId" :nodeId="nodeId" :render-after-expand="renderAfterExpand"
				 :show-checkbox="showCheckbox" :show-radio="showRadio" :check-only-leaf="checkOnlyLeaf" :key="getNodeKey(nodeId)"
