	 ...
	 role="treeitem" @tap.stop="handleClick">
		<view class="ly-tree-node__content" :class="{
			...
			<template v-if="node.icon && node.icon.length > 0">
				<image v-if="node.icon.indexOf('/') !== -1" class="ly-tree-node__icon" mode="widthFix" :src="node.icon" @error="handleImageError">
				</image>
...
<style>
	...
	.ly-tree-node:last-of-type>.ly-tree-node__content{
		border: none;
	}
